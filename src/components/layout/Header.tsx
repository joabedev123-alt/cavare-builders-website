  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-500 bg-brand-black/95 backdrop-blur-md border-b border-brand-dark-border py-3.5 shadow-dark">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav aria-label="Main Navigation" className="hidden md:flex items-center space-x-4 lg:space-x-6 xl:space-x-8">
          {mainNavigation.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                href={item.href}
                className={`text-[11px] lg:text-xs xl:text-sm uppercase tracking-wider xl:tracking-widest transition-colors font-sans py-1 relative ${
                  isActive
